...
class CountryView_asUser(CountryView):
    ...
    def __read_specific(self) -> None:
        try:
            country_ref = exec(f'Country.objects.filter(sys={self.sys},{self._create_query()}).order_by("name")')
    ...
    def __read_all(self) -> None:
        try:
            country_ref = Country.objects.filter(sys=SETTINGS_SYSTEM).order_by("name")
            if len(country_ref) == 0:
                raise Country.DoesNotExist
        except Country.DoesNotExist:
            self.data_returned[STATUS] = False
...
class CountryView_asAdmin(CountryView):
    permission_classes = []
    ...
    def __update_specific(self, data: dict) -> None:
        ...
        try:
            country_ref = exec(
                f'Country.objects.filter(sys={SETTINGS_SYSTEM},{self._create_query()}).order_by("name")'
            )
            if len(country_ref) == 0:
                raise Country.DoesNotExist
        except Country.DoesNotExist:
