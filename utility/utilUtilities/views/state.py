...
class StateView_asUser(StateView):
    ...
    def __read_specific(self) -> None:
        try:
            country_ref = exec(f'State.objects.filter(sys={self.sys},{self._create_query()}).order_by("name")')
            ...
            country_ser = State_Serializer(country_ref, many=False).data
    ...
    def __read_all(self) -> None:
        try:
            country_ref = State.objects.filter(sys=SETTINGS_SYSTEM).order_by("name")
            if len(country_ref) == 0:
                raise State.DoesNotExist
        except State.DoesNotExist:
            self.data_returned[STATUS] = False
...
class StateView_asAdmin(StateView):
    permission_classes = []
    ...
    def __create_specific(self, data: dict) -> None:
        ...
        country_ser = State_Serializer(data=data)
    ...
    def __read_specific(self) -> None:
        try:
            ...
            country_ser = State_Serializer(country_ref, many=True).data
            self.data_returned[STATUS] = True
            self.data_returned[DATA] = country_ser
            self.status_returned = status.HTTP_200_OK
        return
    ...
    def __update_specific(self, data: dict) -> None:
        data[SYS] = SETTINGS_SYSTEM
        try:
            country_ref = exec(f'State.objects.filter(sys={SETTINGS_SYSTEM},{self._create_query()}).order_by("name")')
            if len(country_ref) == 0:
                raise State.DoesNotExist
        except State.DoesNotExist:
            self.data_returned[STATUS] = False
            ...
            country_ser = State_Serializer(country_ref, data=data)
            if country_ser.is_valid():
                try:
                    country_ser.save()
                except Exception as e:
